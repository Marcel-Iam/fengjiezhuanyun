...
Worker 通过本地 `wrangler deploy` 部署，不使用 GitHub 自动部署（自动部署会清空环境变量）。

GitHub repo `Marcel-Iam/fengjiezhuanyun-cloudflareworker` 仅用于版本控制，包含：
- `worker.js` — Worker 源码
- `wrangler.toml` — binding 配置和普通变量

普通变量写在 `wrangler.toml` 的 `[vars]` 里，敏感变量用 `wrangler secret put` 单独设置：
- `GEMINI_API_KEY`
- `WECHAT_CORP_SECRET`
